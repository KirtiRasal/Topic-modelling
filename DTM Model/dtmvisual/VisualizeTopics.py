...
def visualize_topics(df):
    ...
    df.groupby(['period', 'topicId'], sort=False).mean()['distribution'] \
        .unstack().plot(ax=ax,grid=True, linewidth =3.0, sharex=True)
    ...
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), title = "Topics", 
        fontsize='large', labelspacing=0.6, fancybox = True)
